...
- [x] Task 3g: `sync-pages-to-public.mjs` and `prebuild` removed. `/collections/{source}/{source}.json` → `app/api/public-collection/[source]/[file]`, `/config/{site|menu|theme}.json` → `app/api/public-config/[file]` (rewrites `publishedContentRewrites`). Static-boot self-fetch loop resolved at the root: on Next the Save2Repo "published" content is the deployed repo, so `bootSource: 'static'` reads `src/data` from disk like `local` (no same-origin HTTP); `loadStaticPublicPageBundle` deleted. Live check in local and static env: all JSON paths 200/404 as expected, `public/` empty, `verify:webmcp` green
